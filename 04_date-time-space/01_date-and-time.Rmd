...
budget %>% ggplot(aes(x = `Release Date`)) + geom_histogram()
...
#boxbudget$Diff = week(boxbudget$Date) - week(boxbudget$`Release Date`) Not right, why?
#boxbudget$Diff = (boxbudget$Date - boxbudget$`Release Date`) / 7 Seems right, but the format is not good
boxbudget$Diff = as.duration(boxbudget$Date - boxbudget$`Release Date`) / dweeks(1)
```
